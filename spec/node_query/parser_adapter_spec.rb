...
RSpec.describe NodeQuery::ParserAdapter do
  ...
  describe "#is_node?" do
    it 'gets true for node' do
      node = parse("class Rejuvinate; end")
      expect(adapter.is_node?(node)).to be_truthy
    end

    it 'gets false for other' do
      expect(adapter.is_node?("Rejuvinate")).to be_falsey
    end
  end
  ...
  describe "#get_children" do
    it "gets the children of node" do
      node = parse("class Rejuvinate; end")
      expect(adapter.get_children(node)).to eq [parse("Rejuvinate"), nil, nil]
    end
  end

  describe "#get_siblings" do
    it "gets the siblings of node" do
      node = parse("class Rejuvinate; end").children.first

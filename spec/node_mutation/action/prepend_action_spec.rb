...
RSpec.describe NodeMutation::PrependAction do
  describe 'block node without args' do
    subject {
      source = "Rejuvinate::Application.configure do\nend"
      node = Parser::CurrentRuby.parse(source)
      NodeMutation::PrependAction.new(node, 'config.eager_load = true').process
    }

    it 'gets start' do
      expect(subject.start).to eq 'Rejuvinate::Application.configure do'.length
    end

    it 'gets end' do
      expect(subject.end).to eq 'Rejuvinate::Application.configure do'.length
    end
